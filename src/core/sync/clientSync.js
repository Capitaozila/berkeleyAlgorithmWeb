...
function createTimeField(clientId, label, fieldSuffix) {
  return `
    <div class="field border round small">
      <label class="small-margin large-text" for="client${clientId}${fieldSuffix}">
        ${label}:
      </label>
      <input
        type="time"
        id="client${clientId}${fieldSuffix}"
        title="enter the client's ${label.toLowerCase()}"
        placeholder="hh:mm"
      />
    </div>
  `;
...
function createClientCard(client) {
  return `
    <fieldset class="no-border">
      ...
      ${createTimeField(client.id, "local time", "Time")}
      ${createTimeField(client.id, "send time", "SendTime")}
    </fieldset>
  `;
}
...
export function createClientElements() {
  ...
  clients.forEach((client) => {
    ...
    clientDiv.innerHTML = createClientCard(client);
